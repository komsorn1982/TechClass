...
function LegacyDiscoveryFlow({hour,initialStage,onFinish}:{hour:number;initialStage:number;onFinish:()=>void}){
 ...
 function checkWords(){const ok=words.join("|")===definitionWords.join("|");if(ok){setFeedback("ถูกต้อง! เธอประกอบความหมายของโครงงานได้แล้ว");setTimeout(next,750);return}const attempts=wordAttempts+1;setWordAttempts(attempts);if(attempts>=3){setWords([...definitionWords]);setFeedback("ครบ 3 ครั้งแล้ว เฉลยที่ถูกต้องแสดงให้แล้ว ลองอ่านตามลำดับ แล้วกดตรวจความหมายอีกครั้งเพื่อไปต่อ")}else{setFeedback(`ยังไม่ถูก ลองเรียงใหม่อีกครั้ง (${attempts}/3): เริ่มจากอะไร → ใช้อะไร → ทำอย่างไร → สรุปจากอะไร`)}}
