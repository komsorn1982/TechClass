...
export function ProjectDiscoveryFlow(props:{hour:number;initialStage:number;onFinish:()=>void}){
 ...
 if(props.initialStage===0)return <ScienceDetectiveMission hour={props.hour} onComplete={()=>router.push(`/lessons/science-project-m2/lesson/activity/2?hour=${props.hour}`)}/>;
...
function LegacyDiscoveryFlow({hour,initialStage,onFinish}:{hour:number;initialStage:number;onFinish:()=>void}){
 ...
 function checkWords(){
  ...
  if(ok){setWords([...definitionWords]);setWordOutcome("correct");setFeedback("");router.prefetch(nextHref);return}
  const attempts=wordAttempts+1;setWordAttempts(attempts);
  if(attempts>=3){setWords([...definitionWords]);setWordOutcome("revealed");setFeedback("");router.prefetch(nextHref);return}
 ...
 {stage===1&&<div className="sci-flow-panel"><h3>ประกอบความหมายด้วยคำสำคัญ</h3><p>{wordOutcome==="playing"?"แตะคำตามลำดับเพื่อเติมประโยค":"อ่านเฉลยให้เข้าใจ แล้วกดไปต่อเมื่อพร้อม"}</p>{wordOutcome==="playing"?<><div className="sci-definition"><span>โครงงานเริ่มจาก</span><b>{words[0]||"________"}</b><span>ใช้</span><b>{words[1]||"________"}</b><span>ค้นหาคำตอบอย่าง</span><b>{words[2]||"________"}</b><span>และสรุปจาก</span><b>{words[3]||"________"}</b></div><div className="sci-word-bank">{[definitionWords[2],definitionWords[0],definitionWords[3],definitionWords[1]].map(w=><button disabled={words.includes(w)} onClick={()=>addWord(w)} key={w}>{w}</button>)}</div><footer><button onClick={resetWords}>เริ่มเรียงใหม่</button><button disabled={words.length<4} onClick={checkWords}>ตรวจความหมาย</button></footer></>:<><div className={`sci-answer-reveal ${wordOutcome}`}><div className="sci-answer-reveal-head"><span>{wordOutcome==="correct"?"✓":"!"}</span><div><small>เฉลยที่ถูกต้อง</small><strong>{wordOutcome==="correct"?"ตอบถูกต้อง":"ทบทวนเฉลยหลังครบ 3 ครั้ง"}</strong></div></div><div className="sci-answer-sentence"><span>โครงงานเริ่มจาก</span><b>ความสงสัยหรือปัญหา</b><span>ใช้</span><b>กระบวนการทางวิทยาศาสตร์</b><span>ค้นหาคำตอบอย่าง</span><b>เป็นระบบ</b><span>และสรุปจาก</span><b>ข้อมูลและหลักฐานจริง</b></div><div className="sci-answer-steps">{definitionWords.map((w,i)=><div key={w}><i>{i+1}</i><span>{w}</span></div>)}</div></div><footer><button className="sci-flow-check sci-next-button" disabled={navigating} onMouseEnter={()=>router.prefetch(nextHref)} onFocus={()=>router.prefetch(nextHref)} onClick={next}>{navigating?"กำลังไปต่อ...":"ไปต่อ →"}</button></footer></>}</div>}
