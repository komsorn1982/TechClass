...
export async function getCurrentStudent() {
  ...
  const [record]=await getDb().select({id:students.id,firstName:students.firstName,lastName:students.lastName,username:students.username,studentCode:students.studentCode,gradeLevel:students.gradeLevel,classroom:students.classroom,avatarKey:students.avatarKey}).from(sessions).innerJoin(students,eq(sessions.studentId,students.id)).where(and(eq(sessions.tokenHash,await hashSessionToken(token)),gt(sessions.expiresAt,new Date()))).limit(1);
  return record??null;
}
